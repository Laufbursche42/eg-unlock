...
window.I18N = {
  de: {
    ...
    publicLogHelpHtml: "<p>Wenn dies aktiv ist (Standard), wird das Protokoll anonymisiert: Bluetooth-/MAC-Adressen, Geräte-IDs, der Gerätename, Schlüssel und lange Hex-Werte werden geschwärzt. So kannst du das Log gefahrlos teilen oder auf GitHub posten.</p><p>Nur ausschalten, wenn du das vollständige Protokoll <b>lokal</b> für dich brauchst - dann bitte nicht öffentlich teilen.</p>",
    diagLogLabel: "Diagnose-Log (ausführlich)",
    diagLogTitle: "Diagnose-Log",
    diagLogHelpHtml: "<p>Protokolliert zusätzlich ausführliche Details, etwa welche Merkmale abonniert und gelesen werden, plus den Diagnose-Kopf. Nur zur Fehlersuche.</p><p>Personenbezogenes bleibt maskiert, prüfe das Log vor dem Teilen aber selbst.</p>",
  ...
  en: {
    ...
    publicLogHelpHtml: "<p>When this is on (the default), the log is anonymized: Bluetooth / MAC addresses, device IDs, the device name, keys and long hex runs are redacted. So you can safely share it or post it on GitHub.</p><p>Only turn it off if you need the full log <b>locally</b> for yourself - then do not share it publicly.</p>",
    diagLogLabel: "Diagnostic log (verbose)",
    diagLogTitle: "Diagnostic log",
    diagLogHelpHtml: "<p>Also logs verbose details, such as which characteristics are subscribed and read, plus the diagnostic header. For troubleshooting only.</p><p>Personal data stays masked, but review the log yourself before sharing.</p>",
    btnCopyLog: "Copy log",
